...
class InboundRule{
    ...
    //database connection
    private $conn;
    private $table_name = "inbound_rules";
    ...
    public function findWithId() {
        if (isset($this->id)) {
            $query = <<<QUERY
            SELECT id_inbound_rule, inbound_rule_action, port, ip
            FROM $this->table_name
            WHERE id_inbound_rule = :id
            QUERY;
    ...
    /**
     * return inbound rule with action and port set 
    ...
    public function findActionPortIP() {
        ...
        if (isset($this->action) && isset($this->port)) {
            ...
            SELECT id_inbound_rule, inbound_rule_action, port, ip
            FROM $this->table_name
            WHERE inbound_rule_action = :action AND port = :port $ip_set
            QUERY;
    ...
    public function update() {
        ...
        $this->action = isset($this->action) ? htmlspecialchars(strip_tags($this->action)) : $current[ 'inbound_rule_action' ];
    ...
    public function save() {
        ...
        if (!empty($this->id)) {
            ...
            $exist = !empty($exist) && !empty($exist[ 'data' ][ 'id_inbound_rule' ]) ? $exist[ 'data' ][ 'id_inbound_rule' ] : null;
        } 
...
// $ibr = new InboundRule($db);
// $ibr->action = "accept";
// $ibr->port = 444;

// $result = $ibr->findActionPort();
// var_dump($result['data']['id_inbound_rule']);
